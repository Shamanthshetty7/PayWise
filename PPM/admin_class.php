...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM users where username = '".$username."' and pasword = '".$password."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'pasword' && !is_numeric($key))
	...
	function login2(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM users where username = '".$email."' and pasword = '".md5($password)."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
	...
	function save_employee(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("UPDATE employee set ".$data." where EMP_ID=".$id);
		}
		if (!$save) {
	...
	function save_department(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("UPDATE department set ".$data." where DEPT_ID=".$id);
		}
		if($save)
			return 1;
	}
	...
	function save_position(){
		...
		if(empty($id)){
			...
			$save = $this->db->query("UPDATE position set ".$data." where POS_ID=".$id);
		}
		if($save)
			return 1;
	}
	...
	function save_project(){
		extract($_POST);
		$data =" project_name='$ptitle' ";
		$data .=", ALLOWANCE = '$allowance' ";
		if(empty($id)){
			$e_num=date('Y') .'-'. mt_rand(1,9999);
			$data .=", project_num = '$e_num' ";
			$save = $this->db->query("INSERT INTO project set ".$data);
		}else{
			$save = $this->db->query("UPDATE project set ".$data." where Project_ID=".$id);
		}
		if($save)
			return 1;
	}
	function delete_project(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM project where Project_ID = ".$id);
		if($delete)
			return 1;
	}
	...
	function delete_position(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM position where POS_ID = ".$id);
		if($delete)
			return 1;
	}
